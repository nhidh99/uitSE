...
public interface LaptopService {
    ...
    List<LaptopOverviewDTO> findLaptopsByIds(List<Integer> ids);

    LaptopDetailDTO findLaptopDetailById(Integer id);

    LaptopSpecDTO findLaptopSpecById(Integer id);

    Pair<List<LaptopOverviewDTO>, Long> findAndCountLaptopsByFilter(LaptopFilterInput filter);

    Pair<List<LaptopSummaryDTO>, Long> findAndCountLaptopsBySearch(ProductSearchInput search);
}